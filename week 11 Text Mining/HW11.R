...
sba <- scan(mlkspeech, character(0),sep = "\n")
...
#Match words in the speech to AFINN words
...
mlknumberOfWords <- length(mlkfullSpeech)
groups <- ceiling(seq_along(mlkfullSpeech)/(mlknumberOfWords/4))
partsOfSpeech <- split(mlkfullSpeech,groups)
partsOfSpeech
